...
class ActionTodaySchedule(Action):
    ...
    def run(seld,dispatcher,tracker,domain):
        ...
        dispatcher.utter_message(text="Schedule for today:\n" + "\n".join(p for p in list_today))
        return[SlotSet('reminder_list',list_session)]
...
class ActionGetWeather(Action):
    ...
    def run(self,dispatcher,tracker,domain):
        ...
        w_url = "https://api.openweathermap.org/data/2.5/onecall?lat=%s&lon=%s&appid=%s&units=metric"%(tracker.get_slot('origin_lat'),tracker.get_slot('origin_lng'),api_key)
